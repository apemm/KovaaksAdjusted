...
Stability and speed from the old roadmap, the app becoming the place you start KovaaK's from instead of a companion you alt-tab to, and the analysis growing a research-grounded coach.

- **Cited knowledge base + local coach** (`analysis/kb.py`, `analysis/insights.py`): 29 principles, 16 diagnostics, and benchmark threshold tables (Voltaic S3–S5.5, Viscose) distilled from motor-control literature (Fitts, Donovan 2022, Listman 2021, the Challenge Point Framework), Aimer7's guide, and Voltaic docs. A deterministic engine maps each run's metrics through them — every insight card shows the exact numbers that fired it, the sourced interpretation and suggestion, its confidence, and hoverable citations; editorial thresholds say so; bad input health suppresses microstructure claims instead of reporting on noise. Cross-session skill trends (`analysis/skill.py`, Theil-Sen over the saved report history) add plateau-vs-progress framing: falling Fitts slope with a flat score is improvement, and the coach says exactly that.
- **Scenario browser** (Scenarios tab): every local `.sce` with archetype, runs, accuracy, calibration, and last-played; search/filter/sort; Play / Start adapting / Generate variant per row — no more browsing scenarios in the game menu (deep links cannot open local scenarios; the Play flow is playlist-first).
- **The ASCII identity**: the mark is an eye whose iris is a rainbow crosshair, drawn in monospace characters. The opening plays it as an LED matrix — characters warm up as noise, organize into the almond, the iris rainbow-sweeps itself in, the reticle types outward — while a boot worker fits the skill model behind it and narrates. Theme switching sends an ASCII glyph wavefront across the window. Cream-editorial light mode, warm dark twin, four accent presets, all switchable live.
- **Higher calibration ceiling**: "dialed in" now means a settled baseline (24 runs), three observations per wall region, and bias evidence — with staged, per-component progress (runs/regions/bias) shown as counts, not a single opaque percentage.
