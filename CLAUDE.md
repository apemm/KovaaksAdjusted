...
pytest                                # full suite, cross-platform, ~30s
...
- **Never present a number without its evidence.** `gui/dashboard.py:Hero` enforces this structurally (the because-clause is a required constructor field, and `unknown()` still demands one). Cold starts show a dash and say what is missing — never a fake zero, and never a comparison of a run against itself (every EWMA is SEEDED to the first run's own value, so a baseline only exists from run 2).
